 ...
 *
 * Usage:
 *   node .pi/extensions/export-slide.mjs --slide 7
 *   node .pi/extensions/export-slide.mjs --slide 7 --click 2 --output artifacts/slide-7-click-2.png
 *   node .pi/extensions/export-slide.mjs --entry talk.md --slide 7 --click 0
 *
...
Usage:
  node .pi/extensions/export-slide.mjs --slide <number> [options]
...
Options:
  ...
  --server <url>         Use an existing Slidev dev server instead of starting one.
...
Examples:
  node .pi/extensions/export-slide.mjs --slide 12
  node .pi/extensions/export-slide.mjs --slide 12 --click 3 --output artifacts/slide-12.png`)
...
for (let i = 0; i < args.length; i++) {
  ...
  if (!['--slide', '--click', '--entry', '--output', '--server', '--wait', '--timeout'].includes(arg)) {
...
async function exportFromServer(serverUrl) {
  const { chromium } = await import('playwright-chromium')
  const base = new URL(serverUrl)
  if (!base.pathname.endsWith('/')) base.pathname += '/'
  const query = new URLSearchParams()
  if (click !== undefined) query.set('clicks', String(click))
  const url = new URL(`${slide}${query.size ? `?${query}` : ''}`, base)
  ...
  try {
    ...
    await page.goto(url.href, { waitUntil: 'networkidle', timeout })
    const slides = page.locator(`[data-slidev-no="${slide}"]`)
    await slides.first().waitFor({ timeout })
    const target = slides.first()
    if (await target.count() === 0) {
      const available = await slides.evaluateAll(elements => elements.map(element => element.id))
      throw new Error(`Slide ${slide} does not have ${click === undefined ? 'the default state' : `click ${click}`}. Available exported states: ${available.join(', ') || 'none'}.`)
    }
    await target.locator('.slidev-slide-loading').waitFor({ state: 'detached', timeout }).catch(() => {})
    if (wait !== undefined) await new Promise(resolvePromise => setTimeout(resolvePromise, wait))
...
if (options.server !== undefined) {
  try {
    await exportFromServer(options.server)
